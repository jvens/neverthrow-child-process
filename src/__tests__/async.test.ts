...
describe('Async child process wrappers', () => {
  describe('exec', () => {
    ...
    it('should capture stderr in command output', async () => {
      const result = await exec(
        "node -e \"console.error('Error to stderr'); console.log('Output to stdout')\"",
      );
  ...
  describe('spawn', () => {
    ...
    it('should spawn command with both stdout and stderr capture', async () => {
      ...
      expect(result.isOk()).toBe(true);
      if (result.isOk()) {
        const [stdout, stderr] = await Promise.all([result.value.stdout!, result.value.stderr!]);
